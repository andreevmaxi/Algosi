...
struct B_tree{
  ...
  pair<int, int> searchplace(Node_t* now, int num, int n){
     int i = 0;
     pair<int, int> ans;
     ans.first = n;
     ans.second = 0;
     ...
     while(!now->leaf && ans.first < num && i <= now->n){
        tmp = searchplace(now->pointers[i], num, ans.first);
        ans.first += tmp.first + 1;
     ...
     if(ans.second == 0 && ans.first >= num){
        ans.second = now->k[ans.first - num];
     }
  ...
  int kmax(int i){
     pair<int, int> tmp = searchplace(root, i, 0);
